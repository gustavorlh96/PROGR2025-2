...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercicio 6 - Círculo", nullptr, nullptr);
	if (!window) {
	...
	while (!glfwWindowShouldClose(window)) {
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0) {
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 6 - Círculo\tFPS %.2lf", fps);
