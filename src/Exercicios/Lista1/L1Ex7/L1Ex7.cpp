...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercicio 7 - Espiral", nullptr, nullptr);
	if (!window) {
	...
	while (!glfwWindowShouldClose(window)) {
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0) {
				...
				char tmp[256];
				sprintf(tmp, "Exercicio 7 - Espiral\tFPS %.2lf", fps);
...
int createCircle(int nPoints, float radius) {
	...
	int turns = 3; // Número de voltas da espiral
	float angle = 0.0;
	...
	for (int i = 0; i <= nPoints; i++) {
		float t = (float)i / nPoints; // Normaliza i para [0, 1]
		angle = t * turns * 2.0 * pi; // Voltas
		float r = t * radius; // Raio cresce linearmente
		float x = r * cos(angle);
		float y = r * sin(angle);
		float z = 0.0;
	...
	// Garante que o último ponto esteja exatamente no eixo x
	if (vertices.size() >= 6) {
		vertices[vertices.size() - 2] = 0.0; // y do último ponto em 0
    }
